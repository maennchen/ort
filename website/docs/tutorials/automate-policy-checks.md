# Automate your Policy Checks with ORT
...
You're a developer at **Acme Corp**, a mid-size software company. Acme builds a commercial SaaS product with proprietary backend services and also maintains an open-source SDK. The legal team has defined a license compliance policy, and your task is to automate these checks using ORT.

## Acme Corp's license policy
...
For this tutorial, we'll create a custom `license-classifications.yml` that matches Acme's specific policy categories. You typically need a custom file when:
...
categories:
- name: "permissive"
  description: "Licenses with minimal restrictions - allowed at Acme Corp"
- name: "copyleft"
...
Create `acme-rules.kts` with this basic structure:
...
fun RuleSet.unhandledLicenseRule() = packageRule("UNHANDLED_LICENSE") {
    ...
    licenseRule("UNHANDLED_LICENSE", LicenseView.CONCLUDED_OR_DECLARED_AND_DETECTED) {
        ...
        error(
            "The license $license is currently not covered by Acme Corp's policy. " +
...
Acme's proprietary code cannot include GPL or AGPL licensed dependencies. Let's create a matcher and rule for this:
...
LGPL and MPL are allowed at Acme, but static linking can be problematic. Let's warn when a direct dependency is statically linked:
...
Acme requires all high-severity vulnerabilities to be addressed before release:
...
Acme wants every repository to have a README with license information:
...
Here's the complete `acme-rules.kts` file:

```kotlin
// =============================================================================
// Acme Corp License Policy Rules
// =============================================================================
...
Beyond Acme's specific policy, here are some other useful rules you might want to adapt.
...
You now have a working rules file that implements Acme Corp's license policy. From here you can:
...
- Reference
  - [Evaluator rules reference](../reference/configuration/evaluator-rules.md) - Complete DSL reference
  - [Evaluator CLI reference](../reference/cli/evaluator.md) - All CLI options
  - [License classifications](../reference/configuration/license-classifications.md) - Classification file format
- How-to guides
...
- Examples
  - [example.rules.kts](https://github.com/oss-review-toolkit/ort/blob/main/examples/example.rules.kts) - Example rules file
  - [ort-config evaluator.rules.kts](https://github.com/oss-review-toolkit/ort-config/blob/main/evaluator.rules.kts) - Production rules example
